...
function getSavedDocument(editor) {
	...
	if (fileContent !== null) {
		...
		if (fileNameElement !== null) fileNameElement.value = fileName;
	}
}
...
const runCodeForEntity = (element) => {
	if (
		entitiesToolboxInjected &&
		element === entitiesTab &&
		!autoGeneratedEntityBlocks
	) {
		console.log("runCodeForEntity");
		let entityCode =
			getBddGenerator(blockArray).workspaceToCode(entityWorkspace);
...
const runCodeForScenario = (element) => {
	if (
		scenarioToolboxInjected &&
		element === scenarioTab &&
		!autoGeneratedScenarioBlocks
	) {
		console.log("runCodeForScenario");
		let scenarioCode =
			getBddGenerator(blockArray).workspaceToCode(scenarioWorkspace);
...
function onEntityEditorChange() {
	if (
		entities.innerText != null &&
		entities.innerText.replace(/[^a-zA-Z]/g, "").trim() !== ""
	) {
...
if (entitiesTab != undefined) entitiesTab.onclick = switchEditor;
if (scenarioTab != undefined) scenarioTab.onclick = switchEditor;
...
function loadBlocks(element, skipAddingBlocks) {
	fetch("/xtext-service/blocks?resource=multi-resource/scenarios.bdd")
		...
		.then((response) => {
			...
			if (!skipAddingBlocks) Blockly.defineBlocksWithJsonArray(response.blocks);
			...
			Blockly.Blocks["ID"] = {
				init: function() {
					...
					this.appendDummyInput().appendField(
						new Blockly.FieldTextInput("ID", id_validator),
						"TEXT_INPUT"
					);
				},
			};
			...
			Blockly.Blocks["STRING"] = {
				init: function() {
					...
					this.appendDummyInput()
						.appendField('"')
						.appendField(
							new Blockly.FieldTextInput("String", string_validator),
							"TEXT_INPUT"
						)
						.appendField('"');
				},
			...
			response.toolBox.contents.push({
				kind: "category",
				name: "Terminals",
				contents: termArr,
			});
			...
			response.toolBox.contents = filterCategories(
				element,
				originalToolbox.contents
			);

			if (element === scenarioTab && !scenarioToolboxInjected) {
				scenarioWorkspace = Blockly.inject("blockly-editor2", {
			...
			if (element === entitiesTab && !entitiesToolboxInjected) {
				entityWorkspace = Blockly.inject("blockly-editor", {
					toolbox: response.toolBox,
				});
			...
			function onEntityWorkspaceChange(event) {
				...
				if (element === entitiesTab) runCodeForEntity(element);
			}
			...
			function onScenarioWorkspaceChange(event) {
				...
				if (element === scenarioTab) runCodeForScenario(element);
			}
